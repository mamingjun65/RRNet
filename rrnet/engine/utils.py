...
def print_info(epoch, model, optimizer, loss_dict, logger, cfg):
    ...
    if cfg.MODEL.ATTENTION:
        info = "Epoch:{},\t lr:{:6},\t loss_goal:{:.4f},\t loss_revised_goal:{:.4f},\t loss_traj:{:.4f},\t loss_confidence:{:.4f},\t loss_kld:{:.4f},\t \
            kld_w:{:.4f},\t z_clip:{:.4f} ".format( 
            epoch, optimizer.param_groups[0]['lr'], loss_dict['loss_goal'], loss_dict['loss_revised_goal'], loss_dict['loss_traj'], loss_dict['loss_confidence'],
            loss_dict['loss_kld'], loss_dict['kld_weight'], loss_dict['z_logit_clip'])
    else:
        info = "Epoch:{},\t lr:{:6},\t loss_goal:{:.4f},\t loss_traj:{:.4f},\t loss_confidence:{:.4f},\t loss_kld:{:.4f},\t \
            kld_w:{:.4f},\t z_clip:{:.4f} ".format(
            epoch, optimizer.param_groups[0]['lr'], loss_dict['loss_goal'], loss_dict['loss_traj'], loss_dict['loss_confidence'],
            loss_dict['loss_kld'], loss_dict['kld_weight'], loss_dict['z_logit_clip'])
...
def post_process(cfg, X_global, y_global, pred_traj, con_traj, pred_goal=None, dist_traj=None, dist_goal=None):
    ...
    con_traj = con_traj.detach().to('cpu').numpy()
    ...
    return X_global, y_global, pred_goal, pred_traj, dist_traj, dist_goal, con_traj