...
def do_train(cfg, epoch, model, optimizer, dataloader, device, logger=None, lr_scheduler=None):
    ...
    with torch.set_grad_enabled(True):
        for iters, batch in enumerate(tqdm(dataloader), start=1):
            ...
            '''
            with open("test18.txt", "a") as f:
                f.write(str(type(X_global)))
                f.write("=============================================\n")
                f.write("input_x\n")
                f.write(str(X_global.cpu().detach().numpy())+"\n")
                f.write("=============================================\n")
                f.write("input_x_st\n")
                f.write(str(input_x.cpu().detach().numpy())+"\n")
                f.write("=============================================\n")
                f.write("target_y\n")
                f.write(str(y_global.cpu().detach().numpy())+"\n")
                f.write("=============================================\n")
                f.write("target_y_st\n")
                f.write(str(input_y.cpu().detach().numpy())+"\n")
            '''
            pred_goal, pred_traj, con_traj, _, loss_dict, dist_goal, dist_traj, _ = model(input_x,
                                                                                          input_y,
            ...
            with open("input_x.txt", "w") as f:
                f.write(str(X_global.cpu().detach().numpy()) + "\n")
            with open("target_y.txt", "w") as f:
                f.write(str(y_global.cpu().detach().numpy()) + "\n")
            with open("target_y_st.txt", "w") as f:
                f.write(str(input_y.cpu().detach().numpy()) + "\n")
            with open("pred_traj.txt", "w") as f:
                f.write(str(pred_traj.cpu().detach().numpy()) + "\n")
            '''
            if cfg.MODEL.LATENT_DIST == 'categorical':  # gaussian
                ...
                if cfg.MODEL.ATTENTION:
                    if epoch in range(cfg.SOLVER.STAGE1, cfg.SOLVER.STAGE2):
                        loss = loss_dict['loss_revised_goal']
                    if epoch in range(cfg.SOLVER.STAGE2, cfg.SOLVER.STAGE3):
                        loss = loss_dict['loss_traj']
                    if epoch in range(cfg.SOLVER.STAGE3, cfg.SOLVER.MAX_EPOCH):
                        loss = loss + loss_dict['loss_revised_goal'] + loss_dict['loss_traj']
                else:
                    if epoch in range(cfg.SOLVER.STAGE1, cfg.SOLVER.STAGE2):
                        loss = loss_dict['loss_traj']
                    if epoch in range(cfg.SOLVER.STAGE2, cfg.SOLVER.MAX_EPOCH):
                        loss = loss + loss_dict['loss_traj']
...
def do_val(cfg, epoch, model, dataloader, device, logger=None):
    ...
    with torch.set_grad_enabled(False):
        for iters, batch in enumerate(tqdm(dataloader), start=1):
            ...
            pred_goal, pred_traj, con_traj, _, loss_dict, _, _, _ = model(input_x,
                                                                          input_y,
                                                                          neighbors_st=neighbors_st,
                                                                          adjacency=adjacency,
                                                                          cur_pos=X_global[:, -1,
...
def inference(cfg, epoch, model, dataloader, device, logger=None, eval_kde_nll=False, test_mode=False):
    ...
    with torch.set_grad_enabled(False):
        for iters, batch in enumerate(tqdm(dataloader), start=1):
            ...
            pred_goal, pred_traj, con_traj, reconstruct_prob_sm, _, dist_goal, dist_traj, revised_goal = model(input_x,
                                                                                                               neighbors_st=neighbors_st,
                                                                                                               adjacency=adjacency,
                                                                                                               z_mode=False,
                                                                                                               cur_pos=X_global[
                                                                                                               ...
                                                                                                               first_history_indices=first_history_indices)
            if False:
                with open("reconstruct_prob_sm.txt", "a") as f:
                    f.write(str(reconstruct_prob_sm.cpu().detach().numpy()) + "\n")
                with open("con_traj.txt", "a") as f:
                    f.write(str(con_traj.cpu().detach().numpy()) + "\n")
                with open("input_x.txt", "a") as f:
                    f.write(str(input_x.cpu().detach().numpy()) + "\n")
                with open("input_y.txt", "a") as f:
                    f.write(str(input_y.cpu().detach().numpy()) + "\n")
                with open("pred_goal.txt", "a") as f:
                    f.write(str(pred_goal.cpu().detach().numpy()) + "\n")
                with open("revised_goal.txt", "a") as f:
                    f.write(str(revised_goal.cpu().detach().numpy()) + "\n")
...
'''
                ...
                f.write("=============================================\n")
                f.write("X_global\n")
                f.write(str(batch['input_x'].numpy())+"\n")

                f.write("=============================================\n")
                f.write("y_global\n")
                f.write(str(batch['target_y'].numpy())+"\n")
'''