...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
    ...
    x_st = env.standardize(x, state[node.type], node.type, mean=rel_state, std=std)  #减去当前位置，除以3 , std=std

    if list(pred_state[node.type].keys())[0] == 'position':  # If we predict position we do it relative to current pos
        y_st = env.standardize(y, pred_state[node.type], node.type, mean=rel_state[0:2])    #减去当前位置
    else:
    ...
    with open("test9.txt", "a") as f:
        f.write("this is dataloader parameter\n")
        f.write("=============================================\n")
        f.write("X_T\n")
        f.write(str(x_t.numpy())+"\n")
    '''
