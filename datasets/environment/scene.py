...
class Scene(object):
    ...
    def get_clipped_pos_dict(self, timestep, state):
        ...
        for node in existing_nodes:
            pos_dict[node], _ = node.get(tr_scene, {'position': ['x', 'y']})
    ...
    def get_scene_graph(self,
        ...
        if self.temporal_scene_graph is None:
            ...
            for node in present_nodes[timestep]:
                node_pos_dict[node] = np.squeeze(node.get(timestep_range, {'position': ['x', 'y']})[0])
    ...
    def present_nodes(self,
                      timesteps,    #某一场景下，没人-有人的帧数，平移到0，(20,100)->(0,80)
                      type=None,
                      min_history_timesteps=0,  #1/7
                      min_future_timesteps=0,   #12
    ...
    def get_nodes_clipped_at_time(self, timesteps, state):
        ...
        for node in all_nodes:
            ...
            clipped_value = node.get(tr_scene, state[node.type])[0]
