...
def main():
    ...
    #训练结果保存路径"E:\Pycharm\project\rrnet\checkpoints\ETH_UCY_checkpoints\goal_cvae_checkpoints\no_wandb\"
    ...
    if attention:
        #只训练goal1--------------
        for epoch in range(cfg.SOLVER.STAGE1):
        ...
        #冻结goal1参数
        for part in model.parts_to_freeze_goal1:
        ...
        #只训练goal2----------------------
        for epoch in range(cfg.SOLVER.STAGE1,cfg.SOLVER.STAGE2):
            logger.info("Epoch:{}".format(epoch))
            do_train(cfg, epoch, model, optimizer, train_dataloader, cfg.DEVICE, logger=logger, lr_scheduler=lr_scheduler)
            val_loss = do_val(cfg, epoch, model, val_dataloader, cfg.DEVICE, logger=logger)
            if (epoch+1) % 1 == 0:
        ...
        #冻结goal2参数
        for part in model.parts_to_freeze_goal2:
            for param in part.parameters():
                param.requires_grad = False
        #重置学习率
        for param_group in optimizer.param_groups:
        ...
        #只训练traj------------------
        for epoch in range(cfg.SOLVER.STAGE2,cfg.SOLVER.STAGE3):
            logger.info("Epoch:{}".format(epoch))
            do_train(cfg, epoch, model, optimizer, train_dataloader, cfg.DEVICE, logger=logger, lr_scheduler=lr_scheduler)
            val_loss = do_val(cfg, epoch, model, val_dataloader, cfg.DEVICE, logger=logger)
            if (epoch+1) % 1 == 0:
        ...
        # 训练全部---------------------
        for epoch in range(cfg.SOLVER.STAGE3, cfg.SOLVER.MAX_EPOCH):
            logger.info("Epoch:{}".format(epoch))
            do_train(cfg, epoch, model, optimizer, train_dataloader, cfg.DEVICE, logger=logger,
        ...
        # 训练全部------------------
        for epoch in range(cfg.SOLVER.STAGE2, cfg.SOLVER.MAX_EPOCH):
            logger.info("Epoch:{}".format(epoch))
            do_train(cfg, epoch, model, optimizer, train_dataloader, cfg.DEVICE, logger=logger,
                     lr_scheduler=lr_scheduler)
            val_loss = do_val(cfg, epoch, model, val_dataloader, cfg.DEVICE, logger=logger)
